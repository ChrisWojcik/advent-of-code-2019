...
const INPUT_OP = 3;
const OUTPUT_OP = 4;
const JUMP_IF_TRUE_OP = 5;
const JUMP_IF_FALSE_OP = 6;
const LESS_THAN_OP = 7;
const EQUALS_OP = 8;
const HALT_OP = 99;
...
function runProgram(memory, ...inputs) {
  ...
  while (i < memory.length) {
    ...
    if (opCode === ADDITION_OP) {
      ...
      i += 2;
    } else if (opCode === JUMP_IF_TRUE_OP) {
      const param1 = getParameter(param1Mode, i + 1, memory);
      const param2 = getParameter(param2Mode, i + 2, memory);
    ...
    } else if (opCode === JUMP_IF_FALSE_OP) {
      const param1 = getParameter(param1Mode, i + 1, memory);
      const param2 = getParameter(param2Mode, i + 2, memory);
    ...
    } else if (opCode === LESS_THAN_OP) {
      const param1 = getParameter(param1Mode, i + 1, memory);
      const param2 = getParameter(param2Mode, i + 2, memory);

      memory[Number(memory[i + 3])] = (param1 < param2) ? '1' : '0';

      i += 4;
    } else if (opCode === EQUALS_OP) {
      const param1 = getParameter(param1Mode, i + 1, memory);
      const param2 = getParameter(param2Mode, i + 2, memory);
